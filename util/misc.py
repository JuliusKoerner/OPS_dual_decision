...
def filter_unseen_class_oracle(instances, unseen_label_set, category_id=-1):
    gt_classes = instances.gt_classes
    known_idx = []
    unk_idx = []
    for i, c in enumerate(gt_classes):
        if c in unseen_label_set:
            instances.gt_classes[i] = category_id
            unk_idx.append(i)
        else:
            known_idx.append(i)
    return instances, known_idx, instances[unk_idx]
...
def extract_roi_feat(data_path, cls_num=80):
    in_dist_cls_wise_feat = [[] for _ in range(cls_num)]
    out_dist_feat = []
    f_list = os.listdir(data_path)
    for f_name in f_list:
        if '.npy' in f_name:
            roi_feats = np.load(os.path.join(data_path, f_name), allow_pickle=True)[()]
            if len(roi_feats['feat']) > 0:
                for _roi_feat, _label in zip(roi_feats['feat'], roi_feats['label']):
                    in_dist_cls_wise_feat[_label].append(_roi_feat)
            if len(roi_feats['ood_feat']) > 0:
                out_dist_feat.extend(roi_feats['ood_feat'])
    return in_dist_cls_wise_feat, out_dist_feat

def sample_estimator(in_dist_cls_wise_feat, out_dist_feat, num_classes, unseen_label_set):
    """
    ...
    for j in range(num_classes):
        if j in unseen_label_set:
            continue
        _cur_cls_roi_feat = np.concatenate(in_dist_cls_wise_feat[j], 0)
        _cur_cls_roi_feat_mean = np.mean(_cur_cls_roi_feat, 0)
        cls_wise_mean.append(_cur_cls_roi_feat_mean)
        _roi_feat = _cur_cls_roi_feat - _cur_cls_roi_feat_mean
        if combined_cnt == 0:
            whole_feat = _roi_feat
        else:
            whole_feat = np.concatenate((whole_feat, _roi_feat), 0)
        combined_cnt += 1
    
    group_lasso.fit(whole_feat)
    precision = group_lasso.precision_
    return cls_wise_mean, precision

def get_mahalanobis_score(cls_wise_mean, precision, in_dist_cls_wise_feat, out_dist_feat, num_classes, unseen_label_set):
    # out-dist scores
    print("calculate out-dist scores begin")
    Mahalanobis_scores_out_dist = 0
    out_dist_feat = out_dist_feat.squeeze()
    known_idx = 0
    for i in range(num_classes):
        if i in unseen_label_set:
            continue
        _roi_feat_mean = cls_wise_mean[known_idx]
        zero_f = out_dist_feat - _roi_feat_mean
        term_gau = -0.5*np.matmul(np.matmul(zero_f, precision), zero_f.T).diagonal()
        if known_idx == 0:
            Mahalanobis_scores_out_dist = term_gau.reshape(-1,1)
        else:
            Mahalanobis_scores_out_dist = np.concatenate((Mahalanobis_scores_out_dist, term_gau.reshape(-1,1)), 1)
        known_idx += 1
    print("calculate out-dist scores end")

    # in-dist scores
    print("calculate in-dist scores begin")
    Mahalanobis_scores_in_dist_list = []
    for i in range(num_classes):
        if i in unseen_label_set:
            continue
        _cur_cls_roi_feat = np.concatenate(in_dist_cls_wise_feat[i], 0)
        num_sample = _cur_cls_roi_feat.shape[0]
        print(f"in-dist cls {i} has #{num_sample} instance")
        ...
        for j in range(num_classes):
            if j in unseen_label_set:
                continue
            print("#"*30)
            print(f"calculate m-dist to cls {j}")
            print("#"*30)
            _roi_feat_mean = cls_wise_mean[known_idx]
            zero_f = _cur_cls_roi_feat - _roi_feat_mean
            loop_gap = 20000
            if num_sample > loop_gap:
                loop_pivot = 0
                term_gau = 0
                while loop_pivot < num_sample:
                    _tmp_zero_f = zero_f[loop_pivot:loop_pivot+loop_gap]
                    _term_gau = -0.5*np.matmul(np.matmul(_tmp_zero_f, precision), _tmp_zero_f.T).diagonal()
                    if loop_pivot == 0:
                        term_gau = _term_gau.reshape(-1,1)
                    else:
                        term_gau = np.concatenate((term_gau, _term_gau.reshape(-1,1)), 0)
                    loop_pivot += loop_gap
                    print(f"cls {j} loop pivot {loop_pivot}")
            else:       
                term_gau = -0.5*np.matmul(np.matmul(zero_f, precision), zero_f.T).diagonal()
            if known_idx == 0:
                gaussian_score = term_gau.reshape(-1,1)
            else:
                gaussian_score = np.concatenate((gaussian_score, term_gau.reshape(-1,1)), 1)
            known_idx += 1
        Mahalanobis_scores_in_dist_list.append(gaussian_score)

    Mahalanobis_scores_in_dist = np.concatenate(Mahalanobis_scores_in_dist_list, 0)
    print("calculate in-dist scores end")

    Mahalanobis_scores_in_dist_max = np.amax(Mahalanobis_scores_in_dist, 1)
    Mahalanobis_scores_out_dist_max = np.amax(Mahalanobis_scores_out_dist, 1)
    return Mahalanobis_scores_out_dist_max, Mahalanobis_scores_in_dist_max
