...
dataset_dir = os.path.join(os.getenv("DETECTRON2_DATASETS", "datasets"), "coco/annotations/panoptic_val2017.json")
gt_data = json.load(open(dataset_dir,'r'))
id2cat = {}
...
class PQStatCat():
        def __init__(self):
            ...
            # track ingredients of fn
            # self.fn_known_th_gt05 = 0
            # self.fn_known_th_lt05 = 0
            self.fn_known_th_sole_gt05 = 0
            self.fn_known_th_others = 0
            self.fn_unk_lt_iou = 0
            self.fn_st = 0
            # track ingredients of fp
            self.fp_known_th = 0
            self.fp_known_th_others = 0
            self.fp_unk_lt_iou = 0
            self.fp_st = 0
        ...
        def __iadd__(self, pq_stat_cat):
            ...
            # self.fn_known_th_gt05 += pq_stat_cat.fn_known_th_gt05
            # self.fn_known_th_lt05 += pq_stat_cat.fn_known_th_lt05
            self.fn_known_th_sole_gt05 += pq_stat_cat.fn_known_th_sole_gt05
            self.fn_known_th_others += pq_stat_cat.fn_known_th_others

            self.fn_unk_lt_iou += pq_stat_cat.fn_unk_lt_iou
            self.fn_st = pq_stat_cat.fn_st

            self.fp_known_th += pq_stat_cat.fp_known_th
            self.fp_known_th_others += pq_stat_cat.fp_known_th_others
            self.fp_unk_lt_iou += pq_stat_cat.fp_unk_lt_iou
            self.fp_st = pq_stat_cat.fp_st
            return self
...
def pq_compute(gt_json_file, pred_json_file, gt_folder=None, pred_folder=None,
    ...
    # _print_known_stats(pq_stat, categories)
    _print_known_stats_detail(pq_stat, categories)
...
def _print_unk_stats(pq_res, categories):
    headers = ["", "#tp", "#fp", "#fn", "RQ"]
    data = []
    unk_id = [_idx for _idx in np.array(list(pq_res.pq_per_cat.keys())) if _idx < 0]
    for _unk_id in unk_id:
        _unk_name = categories[_unk_id]['name']
        _tp = pq_res[_unk_id].tp
        _fp = pq_res[_unk_id].fp
        _fn = pq_res[_unk_id].fn
        _rq = _tp / (_tp + 0.5 * _fp + 0.5 * _fn)
        row = [_unk_name] + [_tp, _fp, _fn, _rq]
        data.append(row)
        table = tabulate(
            data, headers=headers, tablefmt="pipe", floatfmt=".3f", stralign="center", numalign="center"
        )
    logger.info("Panoptic Evaluation Results for Unkown:\n" + table)

def _print_unk_stats_detail(pq_res, categories):
    # headers = ["", "#tp", "#fp", "#fn", "#fn-known-th-gt0.5", "#fn-known-th-lt0.5","#fn-st", "#fn-unk-lt-iou", "#fp-known-th", "#fp-st", "#fp-unk-lt-iou", "RQ"]
    # headers = ["", "#tp", "#fp", "#fn", "#fn-kn-gt0.5", "#fn-kn-others","#fn-st", "#fn-unk-lt-iou", "#fp-known-th", "#fp-st", "#fp-unk-lt-iou", "RQ"]
    headers = ["", "#tp", "#fp", "#fn", "#fn-kn-gt0.5", "#fn-kn-others","#fn-st", "#fn-unk-lt-iou", "#fp-known-th", "#fp-known-th-others", "#fp-st", "#fp-unk-lt-iou", "RQ", "R", "P", "F1"]
    data = []
    unk_id = [_idx for _idx in np.array(list(pq_res.pq_per_cat.keys())) if _idx < 0]
    for _unk_id in unk_id:
        _unk_name = categories[_unk_id]['name']
        ...
        _fp = pq_res[_unk_id].fp
        _fn = pq_res[_unk_id].fn
        # _fn_known_th_gt05 = pq_res[_unk_id].fn_known_th_gt05
        # _fn_known_th_lt05 = pq_res[_unk_id].fn_known_th_lt05
        _fn_known_th_sole_gt05 = pq_res[_unk_id].fn_known_th_sole_gt05
        _fn_known_th_others = pq_res[_unk_id].fn_known_th_others

        _fn_st = pq_res[_unk_id].fn_st
        _fn_unk_lt_iou = pq_res[_unk_id].fn_unk_lt_iou
        _fp_known_th = pq_res[_unk_id].fp_known_th
        _fp_known_th_others = pq_res[_unk_id].fp_known_th_others
        _fp_st = pq_res[_unk_id].fp_st
        _fp_unk_lt_iou = pq_res[_unk_id].fp_unk_lt_iou
        _rq = _tp / (_tp + 0.5 * _fp + 0.5 * _fn)

        # recall, precision, F1
        _recall = (1.0 * _tp) / (_tp + _fn)
        if _tp + _fp > 0:
            _precision = (1.0 * _tp) / (_tp + _fp)
        else:
            _precision = 0.0
        if _recall + _precision > 0:
            _f1 = 2.0 * _recall * _precision / (_recall + _precision)
        else:
            _f1 = 0.0

        # row = [_unk_name] + [_tp, _fp, _fn, _fn_known_th_gt05, _fn_known_th_lt05, _fn_st, _fn_unk_lt_iou, _fp_known_th, _fp_st, _fp_unk_lt_iou, _rq]
        # row = [_unk_name] + [_tp, _fp, _fn, _fn_known_th_sole_gt05, _fn_known_th_others, _fn_st, _fn_unk_lt_iou, _fp_known_th, _fp_st, _fp_unk_lt_iou, _rq]
        row = [_unk_name] + [_tp, _fp, _fn, _fn_known_th_sole_gt05, _fn_known_th_others, _fn_st, _fn_unk_lt_iou, _fp_known_th, _fp_known_th_others, _fp_st, _fp_unk_lt_iou, _rq, _recall, _precision, _f1]
        data.append(row)
        table = tabulate(
            data, headers=headers, tablefmt="pipe", floatfmt=".3f", stralign="center", numalign="center"
        )
...
def _print_known_stats(pq_res, categories):
    headers = ["", "#tp", "#fp", "#fn", "RQ"]
    data = []
    known_id = [_idx for _idx in np.array(list(pq_res.pq_per_cat.keys())) if _idx > 0]
    for _known_id in known_id:
        if categories[_known_id]['isthing'] != 1:
            continue
        _known_name = categories[_known_id]['name']
        _tp = pq_res[_known_id].tp
        _fp = pq_res[_known_id].fp
        _fn = pq_res[_known_id].fn
        _rq = _tp / (_tp + 0.5 * _fp + 0.5 * _fn)
        row = [_known_name] + [_tp, _fp, _fn, _rq]
        data.append(row)
        table = tabulate(
            data, headers=headers, tablefmt="pipe", floatfmt=".3f", stralign="center", numalign="center"
        )
    logger.info("Panoptic Evaluation Results for Known:\n" + table)

def _print_known_stats_detail(pq_res, categories):
    # print fp of unk pred
    headers = ["", "#tp", "#fp", "#fn", "RQ", "#fp-unk"]
    data = []
    known_id = [_idx for _idx in np.array(list(pq_res.pq_per_cat.keys())) if _idx > 0]
    for _known_id in known_id:
        if categories[_known_id]['isthing'] != 1:
        ...
        _fp = pq_res[_known_id].fp
        _fn = pq_res[_known_id].fn
        _fp_known_th = pq_res[_known_id].fp_known_th
        _rq = _tp / (_tp + 0.5 * _fp + 0.5 * _fn)
        row = [_known_name] + [_tp, _fp, _fn, _rq, _fp_known_th]
        data.append(row)
        table = tabulate(
            data, headers=headers, tablefmt="pipe", floatfmt=".3f", stralign="center", numalign="center"
        )
