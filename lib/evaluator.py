...
from .evaluation_3split import pq_compute_3split, _evaluate_predictions_on_coco_3split, _print_panoptic_results_3split
...
class COCOPanopticOpenEvaluator3Split(COCOPanopticEvaluator):
    """
    Evaluate Panoptic Quality metrics on COCO using PanopticAPI.
    It saves panoptic segmentation prediction in `output_dir`
    ...
    def __init__(self, dataset_name, output_dir, cfg=None):
        ...
        if cfg.DATASETS.UNSEEN_LABEL_SET != '':
            ...
            with open(cfg.DATASETS.UNSEEN_LABEL_SET, 'r') as f:
                self.unknown_label_list = [e.replace('\n', '') for e in f.readlines()]
            assert cfg.DATASETS.TEST_UNSEEN_LABEL_SET != '', 'test unseen label set should not be empty for 3split training!'
            with open(cfg.DATASETS.TEST_UNSEEN_LABEL_SET, 'r') as f:
                self.unseen_label_list = [e.replace('\n', '') for e in f.readlines()]
        else:
            self.unknown_label_list = None
            self.unseen_label_list = None
        # Add unknown class id
        self._thing_contiguous_id_to_dataset_id[-1] = -1
        self._stuff_contiguous_id_to_dataset_id[54] = -1
        self._predictions_json = os.path.join(output_dir, "predictions.json")
    ...
    def evaluate(self):
        ...
        with tempfile.TemporaryDirectory(prefix="panoptic_eval") as pred_dir:
            ...
            with contextlib.redirect_stdout(io.StringIO()):
                pq_res = pq_compute_3split(
                    gt_json,
                    PathManager.get_local_path(self._predictions_json),
                    gt_folder=gt_folder,
                    pred_folder=pred_dir,
                    unknown_label_list=self.unknown_label_list,
                    unseen_label_list = self.unseen_label_list,
                )
        ...
        if "Unknown Things" in pq_res:
            ...
            res["SQ_unknown"] = 100 * pq_res["Unknown Things"]["sq"]
            res["RQ_unknown"] = 100 * pq_res["Unknown Things"]["rq"]
        if "UnSeen Things" in pq_res:
            res["PQ_unseen"] = 100 * pq_res["UnSeen Things"]["pq"]
            res["SQ_unseen"] = 100 * pq_res["UnSeen Things"]["sq"]
            res["RQ_unseen"] = 100 * pq_res["UnSeen Things"]["rq"]
        results = OrderedDict({"panoptic_seg": res})
        _print_panoptic_results_3split(pq_res)
